...
import (
	...
	"github.com/slok/sloth-common-sli-plugins/plugins/deckhouse/latency"
)
...
func TestSLIPlugin(t *testing.T) {
	tests := map[string]struct {
		...
		"Default use case, should return a valid query.": {
			...
			expQuery: `
1 - ((
  sum(rate(ingress_nginx_detail_request_seconds_bucket{ service="test",namespace="default",le="500" }[{{.window}}]))
  /
  (sum(rate(ingress_nginx_detail_request_seconds_count{ service="test",namespace="default" }[{{.window}}])) > 0)
) OR on() vector(1))
`,
		},
		...
		"Using exclude errors, should return a valid query.": {
			...
			expQuery: `
1 - ((
  sum(rate(ingress_nginx_detail_request_seconds_bucket{ code!~"5..",service="test",namespace="default",le="500" }[{{.window}}]))
  /
  (sum(rate(ingress_nginx_detail_request_seconds_count{ code!~"5..",service="test",namespace="default" }[{{.window}}])) > 0)
) OR on() vector(1))
`,
		},
		...
		"Using filter, should return a valid query.": {
			...
			expQuery: `
1 - ((
  sum(rate(ingress_nginx_detail_request_seconds_bucket{ k1="v2",k2="v2",service="test",namespace="default",le="500" }[{{.window}}]))
  /
  (sum(rate(ingress_nginx_detail_request_seconds_count{ k1="v2",k2="v2",service="test",namespace="default" }[{{.window}}])) > 0)
) OR on() vector(1))
`,
		},
		...
		"Using filter and excluding errors, should return a valid query.": {
			...
			expQuery: `
1 - ((
  sum(rate(ingress_nginx_detail_request_seconds_bucket{ k1="v2",k2="v2",code!~"5..",service="test",namespace="default",le="500" }[{{.window}}]))
  /
  (sum(rate(ingress_nginx_detail_request_seconds_count{ k1="v2",k2="v2",code!~"5..",service="test",namespace="default" }[{{.window}}])) > 0)
) OR on() vector(1))
`,
		},
