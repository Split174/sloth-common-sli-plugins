...
const (
	...
	SLIPluginID = "sloth-common/deckhouse/latency"
)

var queryTpl = template.Must(template.New("").Option("missingkey=error").Parse(`
1 - ((
  sum(rate(ingress_nginx_detail_request_seconds_bucket{ {{.filter}}service="{{.service}}",namespace="{{.namespace}}",le="{{.bucket}}" }[{{"{{.window}}"}}]))
  /
  (sum(rate(ingress_nginx_detail_request_seconds_count{ {{.filter}}service="{{.service}}",namespace="{{.namespace}}" }[{{"{{.window}}"}}])) > 0)
) OR on() vector(1))
`))
